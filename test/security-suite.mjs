...
// ==================== N. TOTP 两步验证（0.5.0） ====================
{
  ...
  // 2FA 登录流程（test1 已启用 TOTP；各用独立 IP 避免锁定干扰）
  const loginNoTotp = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'test1', password: '12345678' }), '10.3.0.1'), loginNoTotp)
  await settle()
  check('TOTP', '2FA：密码正确但要求动态码（不签发会话）', loginNoTotp.status === 200 && parseJson(loginNoTotp).totpRequired === true && !(loginNoTotp.headers['set-cookie'] || '').includes('dsh_auth='))
  const login2fa = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'test1', password: '12345678', totp: goodCode }), '10.3.0.2'), login2fa)
  await settle()
  check('TOTP', '2FA：密码 + 动态码两步登录成功', login2fa.status === 200 && /dsh_auth=/.test(login2fa.headers['set-cookie'] || ''))
  const loginBadTotp = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'test1', password: '12345678', totp: '000000' }), '10.3.0.3'), loginBadTotp)
  await settle()
  check('TOTP', '2FA：动态码错误 → 403', loginBadTotp.status === 403)
  const loginFreeTotp = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'test1', totp: goodCode }), '10.3.0.4'), loginFreeTotp)
  await settle()
  check('TOTP', '免密 TOTP 登录成功', loginFreeTotp.status === 200 && /dsh_auth=/.test(loginFreeTotp.headers['set-cookie'] || ''))
  const loginFreeNoTotp = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'admin', totp: '000000' }), '10.3.0.5'), loginFreeNoTotp)
  await settle()
  check('TOTP', '未启用 TOTP 的账号免密 → 400', loginFreeNoTotp.status === 400)
  const loginPlain = makeRes()
  server.emit('request', makeReq('POST', '/auth/login', undefined, JSON.stringify({ username: 'admin', password: 'new-admin-pw-9999' }), '10.3.0.6'), loginPlain)
  await settle()
  check('TOTP', '未启用 TOTP 的账号密码登录照常', loginPlain.status === 200 && /dsh_auth=/.test(loginPlain.headers['set-cookie'] || ''))
