...
const main = async () => {
  ...
  check('注册页含确认密码与眼睛按钮', page.body.includes('确认密码') && page.body.includes('显示/隐藏密码'))
  check('注册页 no-store', (page.headers['cache-control'] || '').includes('no-store'))
  const bad = await retryPost({ username: 'live-reg', password: 'live-reg-pw1', confirmPassword: 'live-reg-pw1', email: '', invite: 'NOPE1234' })
  check('无效邀请码 → 403', bad.status === 403 && JSON.parse(bad.body).error === '邀请码无效或已用完', bad.body.slice(0, 80))
  const mismatch = await retryPost({ username: 'live-reg-m', password: 'live-reg-pw1', confirmPassword: 'different-pw', email: '', invite: 'NOPE1234' })
