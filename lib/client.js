		...
		function AuthUsersPage() {
			...
			_e(function () {
				...
				rpc('totpStatus', {}).then(function (j) {
					if (!cancelled) setTotp({ enabled: j.totp.enabled === true, ignore: j.totp.ignore === true })
				}).catch(function (e) {
					if (e.code !== 'session-expired' && !cancelled) setErr(e.message)
				})
				return function () { cancelled = true }
			...
			function refreshTotp() {
				rpc('totpStatus', {}).then(function (j) {
					setTotp({ enabled: j.totp.enabled === true, ignore: j.totp.ignore === true })
			...
			function genTotp() {
				run(function () {
					return rpc('totpGenerate', {}).then(function (j) {
						setTSecret(j.secret); setTOtpAuth(j.otpauth); setTCode('')
						refreshTotp()
					})
				}, 'TOTP 密钥已生成，请在验证器中添加后输入 6 位动态码启用')
			}
				...
				totp.enabled
					? React.createElement('div', null,
						React.createElement('div', { className: 'muted', style: { marginBottom: 6 } },
							'登录时需输入验证器生成的 6 位动态码（当前版本登录仍用密码；免密 TOTP 登录将在后续版本提供）。'),
					...
					: (tSecret === ''
						...
						: React.createElement('div', null,
							React.createElement('label', null, '密钥（手动添加到验证器）'),
							...
							React.createElement('div', { className: 'row', style: { marginTop: 12 } },
								...
								React.createElement('button', { className: 'ghost', onClick: function () { setTSecret(''); setTOtpAuth(''); setTCode('') }, disabled: busy }, '取消')),
						)),
			))
