...
function AuthUsersPage() {
  ...
  function toggle2fa() {
    run(function() {
      return rpc("totpSet2fa", { enabled: !totp.twoFactor }).then(refreshTotp);
    }, totp.twoFactor ? "已关闭两步验证（密码或动态码任选其一登录）" : "已启用两步验证（登录需密码 + 动态码）");
  }
  function genTotp() {
    run(function() {
  ...
  cards.push(React.createElement(
    ...
    React.createElement(
      "div",
      null,
      totp.enabled ? React.createElement("span", { className: "badge admin" }, "已启用") : React.createElement("span", { className: "badge user" }, "未绑定"),
    ...
    totp.enabled ? React.createElement(
      ...
      React.createElement(
        ...
        React.createElement(
          "label",
          { style: { cursor: "pointer", margin: 0, color: "var(--dsw-alias-label-secondary)" } },
...
function mountSettings(ctx, attempt) {
  ...
  rpc("me", {}).then(function(j) {
    ...
    if (j.me !== void 0 && j.me.totpEnabled !== true && j.me.totpIgnore !== true) {
