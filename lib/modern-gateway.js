 ...
 * legacy dotted/`apiProxy` path in `index.js` is untouched and still serves
  ...
  async function handleHttp(req, res, forward) {
    ...
    if (endpoint === '$events/result') {
      ...
      allowed = object(args) && [...correlations].some(correlation =>
        correlation.login === auth.loginKey(req) && correlation.clientId === args.clientId
    ...
    try {
      ...
      console.error('[dsh-ui-auth] modern gateway projection failed: ' + (error instanceof Error ? error.message : String(error)))
