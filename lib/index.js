...
export function apply(ctx) {
    ...
    function buildStore(c) {
      return {
        ...
        async remove(username) {
          ...
          await c.deleteRecord(SCOPE + '/' + username)
        },
        async readRaw(key) {
    ...
    async function loadSessions() {
      try {
        ...
        for (const [token, s] of Object.entries(data.sessions)) {
          ...
          state.sessions.set(token, { username: s.username, expiresAt: Math.min(s.expiresAt, now + SESSION_TTL_MS) })
          loaded++
        }
    ...
    function createSession(username) {
      ...
      while (state.sessions.has(token)) token = randomHex(24)
      state.sessions.set(token, { username, expiresAt: Date.now() + SESSION_TTL_MS })
      sessionsDirty = true
      persistSessions()
      return token
    }
    ...
    async function setSessionOwner(sessionId, username) {
      state.owners.sessions.set(sessionId, username)
      await persistOwnership()
    }

    async function setWorkspaceOwner(workspaceId, username) {
      state.owners.workspaces.set(workspaceId, username)
      await persistOwnership()
    }
    ...
    function setAuthCookie(res, token) {
      try {
        res.setHeader('set-cookie', COOKIE_NAME + '=' + token + '; Path=/; HttpOnly; SameSite=Strict; Max-Age=' + Math.floor(SESSION_TTL_MS / 1000))
      } catch (err) { /* ignore */ }
    }
    ...
    function loginPage() {
      return '<!DOCTYPE html>' +
        ...
        '</div><script>' +
        '(function(){var f=document.getElementById("f"),e=document.getElementById("e"),b=document.getElementById("b");' +
        ...
        'if(r.status===200&&r.json.ok){location.href=okPath(r.json.redirect)?r.json.redirect:(okPath(next)?next:"/");return}' +
    ...
    async function handleRegister(req, res) {
      ...
      if (req.method === 'POST') {
        ...
        sendJson(res, out.status, out.ok !== undefined ? { ok: true } : { error: out.error })
        return
      }
      sendJson(res, 405, { error: 'method not allowed' })
    }
    ...
    function registerPage() {
      return '<!DOCTYPE html>' +
        ...
        'f.addEventListener("submit",function(ev){ev.preventDefault();b.disabled=true;e.textContent="";o.textContent="";' +
        ...
        '.then(function(r){return r.json().catch(function(){return {}}).then(function(j){return {status:r.status,json:j}})}).then(function(r){' +
        'if(r.status===200&&r.json.ok){o.textContent="注册成功！即将跳转到登录页…";setTimeout(function(){location.href="/auth/login"},1200);return}' +
    ...
    async function handleRpc(req, res, method, body) {
      ...
      switch (method) {
        ...
        case 'totpStatus': {
          sendJson(res, 200, { ok: true, totp: { enabled: me.totpEnabled === true, ignore: me.totpIgnore === true } })
          return
        }
        case 'totpGenerate': {
          ...
          sendJson(res, 200, {
            ok: true,
            secret,
            otpauth: 'otpauth://totp/' + encodeURIComponent('DeepSeek Harness:' + who) +
        ...
        case 'totpRemove': {
          ...
          if (targetName !== who) {
            ...
            if (target === undefined) { sendJson(res, 404, { error: '用户不存在' }); return }
            await storeMutate(targetName, (p) => ({ ...p, totpSecret: undefined, totpEnabled: false }))
          ...
          const code = typeof args.code === 'string' ? args.code.trim() : ''
          if (!totpVerifyCode(me.totpSecret, code)) { sendJson(res, 403, { error: '验证码不正确' }); return }
          await storeMutate(who, (p) => ({ ...p, totpSecret: undefined, totpEnabled: false }))
