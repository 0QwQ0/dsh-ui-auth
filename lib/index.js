...
export function apply(ctx) {
    ...
    function resolveSession(token) {
      ...
      const now = Date.now()
      if (s.expiresAt <= now) { state.sessions.delete(token); return undefined }
    ...
    function invalidateSessions(username, exceptToken) {
      for (const [token, s] of state.sessions) {
        if (s.username === username && token !== exceptToken) state.sessions.delete(token)
      }
    }
    ...
    function clientIp(req) {
      try {
        const addr = req.socket !== undefined ? req.socket.remoteAddress : undefined
    ...
    function loginPage() {
      return '<!DOCTYPE html>' +
        ...
        '</form><div class="foot">访问受保护 · 未授权访问将被拒绝</div>' +
        '</div><script>' +
    ...
    async function handleRpc(req, res, method, body) {
      ...
      const requireAdmin = () => {
        if (me.role !== 'admin') { sendJson(res, 403, { error: '需要管理员权限' }); return false }
        return true
      }
    ...
    ctx.interval(() => {
      const now = Date.now()
      for (const [token, s] of state.sessions) {
        if (s.expiresAt <= now) state.sessions.delete(token)
      }
